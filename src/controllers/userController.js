...
export class UserController {
  ...
  async newUser(req, res) {
    ...
    // Validate input
    if (!password || !SALT_ROUNDS) {
      return res.status(400).json({ error: 'Password and salt rounds are required' });
    }
  
    const saltRounds = parseInt(SALT_ROUNDS, 10);
    if (isNaN(saltRounds)) {
      return res.status(400).json({ error: 'Invalid salt rounds configuration' });
    }

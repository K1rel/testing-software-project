...
export class ProductModel {
  ...
  get(){
    return new Promise((resolve, reject) => {
      this.con.query("SELECT * FROM products", (error, result) => {
        ...
        // Check if result is defined and is an array with items
        if (!result || !Array.isArray(result) || result.length === 0) {
          return reject(new Error("No results found in the database"));
        }
        resolve(result);
      });
    });
  ...
  getNumProducts() {
    return new Promise((resolve, reject) => {
      this.con.query("SELECT COUNT(*) AS numProducts FROM products", (error, result) => {
        ...
        // Handle cases where result is empty
        const numProducts = result && result[0] ? result[0].numProducts : 0;
        resolve(numProducts);
      });
    });
  }
  ...
  static create(product) {
    ...
    const sql = 'INSERT INTO products (title, price, description, stock) VALUES (?, ?, ?)';
    return this.query(sql, [title, price, description, stock]);
  }
}
...
export class UserModel {
        ...
        if (error) {
          if (error.code === 'ER_DUP_ENTRY') {
            reject({
              ...
              sqlMessage: error.sqlMessage // Original SQL error message
            });
          } else {
            reject({
              code: error.code,
  ...
  // static create(user) {
  //   const { fullname, email, password, role } = user; // Add role here
  //   const sql = 'INSERT INTO users (fullname, email, password, role) VALUES (?, ?, ?, ?)';
  //   return this.query(sql, [fullname, email, password, role]);
  // }
}